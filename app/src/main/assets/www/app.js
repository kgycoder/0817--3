...
function closeNP() {
    ...
    // Lyrics View가 열려 있었다면 함께 닫아 다음에 NP를 열 때 깨끗한 상태로 시작
    if (LV.active) _closeLyricsView();
    document.getElementById('np').classList.remove('on');
    _stopLyricsTick();
}
...
function _renderLyrics() {
    ...
    if (LV.active) {
        _syncLvHead();
        _buildLvLyricsDOM();
        requestAnimationFrame(() => _highlightLvLine(LY.curIdx));
        _ensureLyricsTranslation();
    }
}
...
function _showLyricsLoading() {
    ...
    if (LV.active) _showLvEmpty('가사 불러오는 중...');
}
...
function _showLyricsEmpty() {
    ...
    if (LV.active) _showLvEmpty('가사를 찾을 수 없습니다');
}
...
function _clearLyrics() {
    ...
    if (NP_LS._scrollSpring) { NP_LS._scrollSpring.stop(); NP_LS._scrollSpring = null; }
    // Lyrics View(신규) 정리 — 뷰가 열려 있으면 로딩 상태로 되돌리고, 다음 트랙 가사가
    // 오면 _renderLyrics()/_showLyricsLoading()/_showLyricsEmpty()가 다시 채운다.
    const lvInner = document.getElementById('np-lv-inner');
    if (lvInner) lvInner.innerHTML = '';
    document.getElementById('np-lv-empty')?.remove();
    if (LV._spring) { LV._spring.stop(); LV._spring = null; }
    LY.lines = [];
    LY.curIdx = -1;
    LY.videoId = null;
    LY._fetching = null;
...
/* ════════════════════════════════════════════
   LYRICS VIEW — Apple Music 스타일 전체 가사 화면
   (Android NP 전용, PC #np.fullscreen / 가로모드 #np.np-landscape
   시스템과 완전히 독립적인 신규 레이어. 기존 인라인 가사 패널의
   폰트(--lyric-font)와 스프링 이징(_makeSpring)만 그대로 인용한다.)
════════════════════════════════════════════ */
const LV = { active: false, _spring: null };

function toggleLyricsView() {
    // 가로모드는 자체 전용 레이아웃을 쓰므로 Lyrics View와 동시 사용하지 않음
    if (NP_LS.active) return;
    if (LV.active) _closeLyricsView();
    else _openLyricsView();
}
...
function _openLyricsView() {
    ...
    document.getElementById('np-lyrics-toggle-btn')?.classList.add('on');
    _syncLvHead();
    if (LY.lines.length > 0) {
        _buildLvLyricsDOM();
        requestAnimationFrame(() => _highlightLvLine(LY.curIdx));
        ...
        _showLvEmpty(LY._fetching ? '가사 불러오는 중...' : '가사를 찾을 수 없습니다');
    }
}
...
function _closeLyricsView() {
    ...
    document.getElementById('np-lyrics-toggle-btn')?.classList.remove('on');
}

function _syncLvHead() {
    if (!S.track) return;
    const titleEl = document.getElementById('np-lv-title');
    const chEl = document.getElementById('np-lv-ch');
    if (titleEl) titleEl.textContent = S.track.title || '';
    if (chEl) chEl.textContent = S.track.channel || 'YouTube';
}

function _showLvEmpty(msg) {
...
/* 거리(d = i - 현재줄index)에 따라 불투명도/블러/스케일을 부여 —
   기존 전체화면(np-fs) 시스템의 FS_SLOTS 감쇠 곡선과 동일한 값 사용 */
function _lvSlotFor(d) {
    const ad = Math.abs(d);
    if (d === 0) return { op: 1,    bl: 0,   sc: 1 };
    if (ad === 1) return { op: 0.60, bl: 0.6, sc: 0.96 };
    if (ad === 2) return { op: 0.36, bl: 1.6, sc: 0.92 };
    if (ad === 3) return { op: 0.20, bl: 2.8, sc: 0.90 };
    return { op: 0.12, bl: 4.0, sc: 0.88 };
}

function _highlightLvLine(idx) {
    if (!LV.active || !LY.lines.length) return;
    const inner = document.getElementById('np-lv-inner');
    const scroll = document.getElementById('np-lv-scroll');
    ...
    LY.lines.forEach((line, i) => {
        ...
        const d = i - idx;
        if (Math.abs(d) > 7) {
            el.style.transition = 'opacity .4s var(--lyric-fade), filter .4s var(--lyric-fade)';
            el.style.opacity = '0';
            el.style.filter = 'blur(8px)';
            return;
        }
        const slot = _lvSlotFor(d);
        el.style.transition = [
            'opacity .4s var(--lyric-fade)',
            'filter .4s var(--lyric-fade)',
            'color .4s var(--lyric-fade)',
            'transform .58s var(--lyric-spring-move)'
        ].join(', ');
        el.style.opacity = String(slot.op);
        el.style.filter = slot.bl > 0 ? `blur(${slot.bl}px)` : 'none';
        el.style.color = d === 0 ? 'rgba(255,255,255,0.97)' : `rgba(255,255,255,${Math.max(0.18, slot.op * 0.62)})`;
        el.style.transform = `scale(${slot.sc})`;
    });

    if (idx >= 0 && LY.lines[idx]?.el_lv) {
        const el = LY.lines[idx].el_lv;
        const target = (scroll.clientHeight / 2) - (el.offsetTop + el.offsetHeight / 2);
        if (!LV._spring) LV._spring = _makeSpring();
        LV._spring.to(target, v => { inner.style.transform = `translateY(${v}px)`; });
    }
}

/* ── 가사 번역 (Apple Music 스타일: 외국어 가사일 때만 원문 아래 작게 표시) ──
   API 키 없이 WebView에서 바로 호출 가능한 Google 번역 비공식 엔드포인트 사용 */
const TR_CACHE = {}; // videoId -> { pending, done, foreign }

function _looksKorean(text) {
    const hangul = (text.match(/[\uac00-\ud7a3\u3131-\u318e]/g) || []).length;
    const letters = (text.match(/[A-Za-z\uac00-\ud7a3\u3131-\u318e\u3040-\u30ff\u4e00-\u9fff]/g) || []).length;
    if (letters === 0) return true; // 판단 불가 → 번역 생략
    return hangul / letters > 0.35;
}

async function _translateLines(texts, tl = 'ko') {
    ...
    const segs = (data && data[0] ? data[0] : []).map(s => s[0] || '');
    const merged = segs.join('').split('\n');
    if (merged.length !== texts.length) throw new Error('line count mismatch');
    return merged;
}

async function _ensureLyricsTranslation() {
    const vid = LY.videoId;
    if (!vid || !LY.lines.length) return;
    const cached = TR_CACHE[vid];
    if (cached?.done) { if (cached.foreign) _applyLvTranslations(); return; }
    if (cached?.pending) return;
    TR_CACHE[vid] = { pending: true };
    try {
        const sample = LY.lines.slice(0, 8).map(l => l.text).join(' ');
        if (_looksKorean(sample)) {
            TR_CACHE[vid] = { done: true, foreign: false };
            return;
        }
        const CHUNK = 40;
        for (let i = 0; i < LY.lines.length; i += CHUNK) {
            if (LY.videoId !== vid) { delete TR_CACHE[vid]; return; } // 트랙 변경 시 중단
            const chunk = LY.lines.slice(i, i + CHUNK);
            const translated = await _translateLines(chunk.map(l => l.text));
            translated.forEach((tr, j) => { if (chunk[j]) chunk[j].tr = tr.trim(); });
        }
        if (LY.videoId !== vid) { delete TR_CACHE[vid]; return; }
        TR_CACHE[vid] = { done: true, foreign: true };
        _applyLvTranslations();
    } catch (err) {
        TR_CACHE[vid] = { done: true, foreign: false };
    }
}

function _applyLvTranslations() {
    if (!LV.active || LY.videoId == null) return;
    let changed = false;
    LY.lines.forEach(line => {
        if (!line.tr || !line.el_lv || line.el_lv.querySelector('.np-lv-line-tr')) return;
        const tr = document.createElement('span');
        tr.className = 'np-lv-line-tr';
        tr.textContent = line.tr;
        line.el_lv.appendChild(tr);
        changed = true;
    });
    if (changed) requestAnimationFrame(() => _highlightLvLine(LY.curIdx));
}
